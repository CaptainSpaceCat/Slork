...
Cricket2 cricks[30];
for (int i; i < 30; i++) {
    cricks[i].set(Std.mtof(base_note - 1*(i%5)), 350::ms, 500::ms, 0.2);
...
for (int i; i < 100; i++) {
    Std.mtof(base_note + Math.random2(-5, 5)) => float curr_freq;
    for (int i; i < 3; i++) {
        cricks[i].set_freq(curr_freq);
    }
    1::second => now;
}
...
class Cricket2
{
    ...
    public void set(float _freq, dur _len, dur _delay, float _gain) {
        _gain => t.gain;
        _freq => t.freq;
        _len => len;
        _delay => delay;
